...
def _make_sign(params: dict[str, str]) -> str:
    """Generate HMAC-SHA256 signature for Shopline API requests."""
...
def verify_hmac(params: dict) -> bool:
    """Verify Shopline HMAC-SHA256 signature on incoming requests."""
...
async def callback(code: str, handle: str):
    ...
    # POST signing: source = body_string + timestamp
    ...
    # Parse expireTime (ISO format like 2023-11-10T16:37:48.178+00:00)
    ...
    # Also create subscription record if not exists
...
async def reauth_url(handle: str = ""):
    ...
    if not handle:
        # Fallback: get the first store from DB
        from backend.db.connection import get_connection
        with get_connection() as conn:
            with conn.cursor() as cur:
