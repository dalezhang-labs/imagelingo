...
async def get_job_status(job_id: str):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT status, error_msg FROM imagelingo.translation_jobs WHERE id = %s", (job_id,))
            job = cur.fetchone()
            if not job:
            ...
            rows = cur.fetchall()
    return {"status": status, "results": {lang: url for lang, url in rows}, "error": error_msg}
