...
def main():
    ...
    # Step 1: Get image URL (upload local file if needed)
    if not image_url:
    ...
    # Step 2: OCR (optional — enhances Lovart prompt)
    print(f"\n[2/3] Running EasyOCR...")
    ocr_texts: list[str] = []
    try:
        ...
        if results:
            ...
            print("  No text regions found (EasyOCR may not be installed or image has no detectable text)")
            print("  Pipeline will continue — Lovart can detect text independently")
